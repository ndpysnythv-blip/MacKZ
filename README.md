...
| **手机遥控（演示用）** | `RemoteControl.swift` | Network.framework 起一个仅监听局域网的**纯 HTTP** 服务，手机经官网配对页（`kdxzhx.top/mackz-pair`，连接码 + 二维码）或直接打开局域网地址，即可「合上 / 打开 / 播放一次 / 拖进度」；每次启动随机生成 6 位口令，关闭开关即完全停止。手机页面还内置陀螺仪铰链模式，但 iOS 只允许 https 页面读运动传感器，纯 HTTP 下取不到数据 |
...
2. 点「打开配对页」→ 浏览器打开官网配对页 <https://kdxzhx.top/mackz-pair>，
   页面上会显示大号连接码和二维码；
...
### v1.7.8
- **新增「官网配对页」连接手机**：Mac 面板点「打开配对页」→ 浏览器打开
  <https://kdxzhx.top/mackz-pair>，页面显示大号连接码 + 二维码；手机连同一个 Wi-Fi，
  扫码或用连接码配对后直达控制页。
