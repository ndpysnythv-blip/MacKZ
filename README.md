...
| **手机遥控（演示用）** | `RemoteControl.swift` | Network.framework 起一个仅监听局域网的**纯 HTTP** 服务，手机浏览器打开设置面板显示的地址即可「合上 / 打开 / 播放一次 / 拖进度」；每次启动随机生成口令 `t=xxxx`，关闭开关即完全停止。手机页面还内置陀螺仪铰链模式，但 iOS 只允许 https 页面读运动传感器，纯 HTTP 下取不到数据 |
...
| 手机遥控（演示用） | 显示局域网访问地址 + 复制链接 / 刷新地址；「启用手机遥控」「允许手机陀螺仪接管角度」两个开关；长说明收进一行蓝色链接「更多介绍」 |
...
- `t=xxxx` 是每次启动随机生成的 6 位口令，只在局域网内有效；换 Wi-Fi 或 IP 变了点「刷新地址」重新生成；
