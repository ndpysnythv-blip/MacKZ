...
| 手机遥控（演示用） | 显示局域网访问地址（HTTPS）+ 复制链接 / 在本机打开 / 刷新地址、重新检测连接 / 打开本地网络设置，遥控开关，以及「允许手机陀螺仪接管角度」开关 |
...
- 启动后会自动自检「回环 + 局域网」两段路径，状态行会直接说明卡在哪一段；手机打不开时点「重新检测连接」复检。

### 手机打不开怎么办（按出现频率排序）

1. **没给「本地网络」权限**：macOS 15 起 App 访问局域网需要授权，未授权时手机连不上 Mac。
   点设置面板的「打开本地网络设置」，勾选 MacKZ；
2. **Wi-Fi 隔离**：很多路由器的「访客网络 / AP 隔离」会禁止同一 Wi-Fi 下设备互访，换到主网络即可；
3. **地址过期**：Mac 换了 Wi-Fi 或 DHCP 续租后 IP 会变，点「刷新地址」重新生成链接（口令也会换）；
4. **HTTPS 证书提示**：手机首次打开要点「显示详细信息 → 继续访问」，否则页面不会加载；
5. 仍然不行就看状态行：如果提示「监听异常」，换个端口（高级设置 → 手机遥控端口）后点「重新检测」。
