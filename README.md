...
| `phoneGyro` | true | 是否允许手机陀螺仪接管铰链角度（手机贴屏幕上模拟铰链，见第 15 节）。服务本身固定优先 HTTPS |
...
| 手机遥控（演示用） | 显示局域网访问地址 + 复制链接 / 刷新地址；「启用手机遥控」「陀螺仪模式（HTTPS）」两个开关；长说明收进可折叠的「使用说明 / 打不开时的排查」 |
...
- **固定优先走 HTTPS**：自签证书由 `RemoteTLS.swift` 现场生成（标准服务器证书，含本机 IP 的 SAN）。
  手机陀螺仪必须走 HTTPS（安全上下文），Safari 也会把访问过的地址记成「必须 HTTPS」，
  所以只有证书在本机不可用时才会回退 HTTP（状态行会说明）；
- 启动时会自动自检「回环（验证监听与 TLS 握手）+ 局域网（验证手机那条路）」两段，结论写在面板状态行。

### 手机打不开怎么办（按出现频率排序）

1. **没点「继续访问」**：自签证书必须手动放行一次，否则页面不会加载；
2. **Safari 报「已启用『仅限 HTTPS』」**：该地址已被 Safari 记成必须 HTTPS ——
   用面板上的 https 地址，或到「设置 → Safari → 高级」关掉「仅限 HTTPS」；
3. **macOS 防火墙拦了传入连接**：「系统设置 → 网络 → 防火墙 → 选项」里允许 MacKZ 接受传入连接；
4. **macOS 15+ 没给「本地网络」权限**：「隐私与安全性 → 本地网络」里允许 MacKZ；
5. **不在同一个 Wi-Fi / 访客网络隔离**：很多路由器的「访客网络 / AP 隔离」会禁止同网段设备互访；
6. **地址过期**：IP 变了点「刷新地址」重新生成链接（口令也会换）。
...
### v1.7.6
- **手机遥控重新固定走 HTTPS**。如果你看到 Safari 报「导览失败，因为要求是针对已启用『仅限 HTTPS』的 HTTP URL」，
  说明该地址已经被 Safari 记成「必须 HTTPS」（此前用 https 成功打开过一次就会这样），再用 http 必然被拦。
  现在服务固定优先以 HTTPS 起监听，拿不到可用证书才回退 HTTP 并在状态行说明。
- **自签证书按标准服务器证书签发**：补 `extendedKeyUsage=serverAuth` + `keyUsage`，`basicConstraints` 改为 `CA:FALSE`。
  缺少 EKU 时 Apple 的 TLS 栈可能直接拒绝握手，手机端连「继续访问」的证书页都出不来。
- **PKCS#12 导出改为逐组参数尝试**（3DES/SHA1 → legacy → 默认），并以「Security 能否真的导入」作为成功判据；
  以前只要导入失败就静默回退 HTTP —— 这是「手机打不开」的一个隐藏原因，现在会写进日志与状态行。
- **自检不再误降级**：HTTPS 首次握手偶尔偏慢，现在重试一次再决定是否回退，避免可用 HTTPS 被误降级。
