...
| **手机遥控（演示用）** | `RemoteControl.swift` | Network.framework 起一个仅监听局域网的**纯 HTTP** 服务，手机经介绍页 <https://kdxzhx.top/mackz> 的「手机遥控配对」区块（连接码 + 二维码）或直接打开局域网地址，即可「合上 / 打开 / 播放一次 / 拖进度」；每次启动随机生成 6 位口令，关闭开关即完全停止。手机页面还内置陀螺仪铰链模式，但 iOS 只允许 https 页面读运动传感器，纯 HTTP 下取不到数据 |
...
1. 设置面板 →「手机遥控（演示用）」，确认开关是打开的，面板上会显示形如
   `192.168.1.5:52800#836291` 的**连接码**（= Mac 局域网地址 + 本次口令）；
2. 点「打开配对页」→ 浏览器打开介绍页 <https://kdxzhx.top/mackz> 里的「手机遥控配对」区块，
   页面上会显示大号连接码和二维码；
3. 手机连同一个 Wi-Fi：**扫码**即可直达控制页；扫不了时在配对页输入框里粘贴连接码也一样
   （Mac 面板点「复制连接码」可以自己发到手机上）；
4. 控制页是大字按钮页面：**合上 / 打开 / 播放一次开合 / 复位**，外加一个实时进度滑杆。

细节说明：
- 服务基于 `Network.framework`，**只监听局域网、不对外联网、不写任何文件**，关掉开关即完全停止；
- 连接码里的 6 位数字是每次启动随机生成的口令，只在局域网内有效；换 Wi-Fi 或 IP 变了点「刷新地址」重新生成；
- 换端口在「高级设置 → 手机遥控端口」里改（默认 `52800`）；
- **固定使用纯 HTTP**：v1.7.6 曾为手机陀螺仪改用自签证书 HTTPS，但导入 PKCS#12 会把私钥写进
  「登录钥匙串」，每次启动都弹钥匙串授权框且卡住主线程，因此 v1.7.7 起彻底回到纯 HTTP；
- **官网只做配对入口**：浏览器硬性禁止 https 页面直接请求局域网 http 服务（混合内容会被拦），
  所以控制页必须由 Mac 本地提供，配对页负责把手机「送」过去（顶层跳转不受该限制）；
  连接码放在 URL 的 `#` 片段里，片段不会发往服务器，只在你本机浏览器内解析；
- 启动时会自动自检「回环（验证监听）+ 局域网（验证手机那条路）」两段，结论写在面板状态行。
