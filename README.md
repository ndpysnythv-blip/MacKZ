...
| `phoneGyro` | false | **陀螺仪模式**：true 时手机遥控服务改用 HTTPS（读取运动传感器必须安全上下文）；默认 false 走 HTTP |
...
- **默认走 HTTP**（兼容性最好、手机上不需要任何证书确认）；打开设置里的「陀螺仪模式（HTTPS）」后
  才会用 `RemoteTLS.swift` 现场签的自签证书起 HTTPS —— 遥控按钮两种模式都能用，
  但**手机陀螺仪必须走 HTTPS**（浏览器只在安全上下文下开放运动传感器）；
- 启动时会自动自检「回环（验证监听与 TLS 握手）+ 局域网（验证手机那条路）」两段，结论写在面板状态行。

### 手机打不开怎么办（按出现频率排序）

1. **不在同一个 Wi-Fi / 访客网络隔离**：很多路由器的「访客网络 / AP 隔离」会禁止同网段设备互访；
2. **macOS 防火墙拦了传入连接**：「系统设置 → 网络 → 防火墙 → 选项」里允许 MacKZ 接受传入连接；
3. **macOS 15+ 没给「本地网络」权限**：「隐私与安全性 → 本地网络」里允许 MacKZ；
4. **地址过期**：Mac 换了 Wi-Fi 或 DHCP 续租后 IP 会变，点「刷新地址」重新生成链接（口令也会换）；
5. **用了 HTTPS 但没点「继续访问」**：手机首次打开自签证书页面必须点「显示详细信息 → 继续访问」。
